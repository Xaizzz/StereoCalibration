...
{
    /// <summary>
    /// Форма выбора и предварительного просмотра камер.
    /// 
    /// Исторически файл называется Form1.cs, но реальный класс — CameraSelectionForm.
    /// Форма открывается из MainForm при старте приложения, позволяет выбрать
    /// две разные камеры, проверить их предпросмотром и вернуть выбранные индексы.
    /// 
    /// Важно: предпросмотр реализован здесь автономно через VideoCapture, а не через
    /// CameraPreviewService. Поэтому при закрытии формы ресурсы камер освобождаются
    /// особенно тщательно, чтобы MainForm мог открыть те же устройства заново.
    /// </summary>
    public partial class CameraSelectionForm : Form
    {
        #region Публичные свойства
        /// <summary>Индекс выбранной первой камеры</summary>
        public int Camera1Index { get; private set; }
        /// <summary>Индекс выбранной второй камеры</summary>
        public int Camera2Index { get; private set; }
        #endregion

        #region Управление камерами
        /// <summary>Объект захвата первой камеры</summary>
        private VideoCapture capture1;
        /// <summary>Объект захвата второй камеры</summary>
        private VideoCapture capture2;
        /// <summary>Таймер для обновления предпросмотра камер</summary>
        private System.Windows.Forms.Timer timer;
        #endregion

        #region Состояние предпросмотра
        /// <summary>Флаг активности предпросмотра первой камеры</summary>
        private bool isPreviewingCamera1 = false;
        /// <summary>Флаг активности предпросмотра второй камеры</summary>
        private bool isPreviewingCamera2 = false;
        
        /// <summary>Токен отмены операций подключения к первой камере</summary>
        private System.Threading.CancellationTokenSource camera1CancelToken;
        /// <summary>Токен отмены операций подключения ко второй камере</summary>
        private System.Threading.CancellationTokenSource camera2CancelToken;
        
        /// <summary>Режим последовательного выбора (сначала камера 1, затем камера 2)</summary>
        private bool sequentialMode = true;
        #endregion

        #region Цветовая схема интерфейса (Apple-style)
        /// <summary>Основной цвет фона</summary>
        private Color primaryBackground = Color.FromArgb(248, 249, 250);
        /// <summary>Вторичный цвет фона</summary>
        private Color secondaryBackground = Color.White;
        /// <summary>Акцентный цвет для кнопок и элементов</summary>
        private Color accentColor = Color.FromArgb(0, 122, 255);
        /// <summary>Цвет текста</summary>
        private Color textColor = Color.FromArgb(29, 29, 31);
        /// <summary>Цвет границ элементов</summary>
        private Color borderColor = Color.FromArgb(209, 213, 219);
        #endregion

        /// <summary>
        /// Безопасное отображение сообщений об ошибках в UI потоке
        /// </summary>
        ...
        /// <summary>
        /// Создаёт окно выбора камер и заполняет ComboBox найденными индексами.
        /// 
        /// В последовательном режиме сначала проверяется первая камера, после этого
        /// разблокируется выбор и предпросмотр второй. Это снижает шанс выбрать два
        /// одинаковых устройства и упрощает пользовательский сценарий.
        /// </summary>
        ...
        private async void btnPreviewCamera2_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    btnPreviewCamera2.BackColor = System.Drawing.Color.FromArgb(29, 29, 31);
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Ошибка предпросмотра камеры 2: {ex.Message} (проверьте, доступна ли камера и нет ли конфликта индексов).", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                StopPreviewCamera2();
                isPreviewingCamera2 = false;
                btnPreviewCamera2.Text = "Предпросмотр";
            }
        ...
        /// <summary>
        /// Асинхронно открывает первую камеру и запускает её предпросмотр.
        /// 
        /// Метод выполняет несколько попыток открытия и проверочный Read, потому что
        /// USB-камеры не всегда становятся доступными сразу после создания VideoCapture.
        /// </summary>
        ...
        /// <summary>
        /// Останавливает предпросмотр первой камеры и освобождает VideoCapture.
        /// В последовательном режиме также сбрасывает вторую камеру, потому что её
        /// выбор зависит от успешной проверки первой.
        /// </summary>
        ...
        /// <summary>
        /// Асинхронно открывает вторую камеру и запускает её предпросмотр.
        /// Логика почти повторяет первую камеру, но работает со вторым набором
        /// UI-элементов и отдельным токеном отмены.
        /// </summary>
        ...
        /// <summary>
        /// Останавливает предпросмотр второй камеры и освобождает её VideoCapture.
        /// </summary>
        ...
        /// <summary>
        /// Таймер предпросмотра. Раз в 100 мс обновляет только те PictureBox,
        /// для которых сейчас включён предпросмотр.
        /// </summary>
        ...
        /// <summary>
        /// Читает один кадр из VideoCapture и безопасно заменяет изображение PictureBox.
        /// Старый Bitmap обязательно освобождается, иначе при живом видео быстро
        /// накапливаются GDI-объекты.
        /// </summary>
        ...
        /// <summary>
        /// Валидирует выбор камер, останавливает предпросмотр и закрывает форму с OK.
        /// MainForm затем берёт Camera1Index/Camera2Index и открывает камеры уже
        /// в основном StereoCameraService.
        /// </summary>
        ...
        /// <summary>
        /// Финальная очистка ресурсов формы выбора камер.
        /// 
        /// Здесь отменяются подключения, останавливается таймер, освобождаются
        /// VideoCapture и изображения. Принудительный GC оставлен как практическая
        /// мера против зависания USB-камер после предпросмотра.
        /// </summary>
        ...
        private System.Windows.Forms.ComboBox comboBoxCamera1 = null!;
        private System.Windows.Forms.ComboBox comboBoxCamera2 = null!;
        private System.Windows.Forms.Button btnApply = null!;
        private System.Windows.Forms.Button btnPreviewCamera1 = null!;
        private System.Windows.Forms.Button btnPreviewCamera2 = null!;
        private System.Windows.Forms.Label label1 = null!;
        private System.Windows.Forms.Label label2 = null!;
        private System.Windows.Forms.PictureBox pictureBoxCamera1 = null!;
        private System.Windows.Forms.PictureBox pictureBoxCamera2 = null!;
        private System.Windows.Forms.Label labelCamera1 = null!;
        private System.Windows.Forms.Label labelCamera2 = null!;
        private System.Windows.Forms.Label labelInstruction = null!;

        /// <summary>
        /// Ручная сборка интерфейса выбора камер.
        /// Designer-файла для этой формы нет: все ComboBox, PictureBox, кнопки
        /// и декоративные элементы создаются программно.
        /// </summary>
        private void InitializeComponent()
        {
            ...
            // Apple-style design colors are now class fields
